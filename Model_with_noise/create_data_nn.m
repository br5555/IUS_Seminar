...
for i = 1:length(radijus_kruz)
    x_0 = 0.03 + (0.36-0.03)*rand();
    y_0 = 0.03 + (0.46-0.03)*rand();
    disp('kruznica')
    i
    [u_x_values, u_y_values, y_x_values, y_y_values] = simuliraj_kruznicu(radijus_kruz(i),x_0, y_0);
    C = mat2cell(y_x_values(1:end)',1, ones(length(y_x_values),1));
    C1 = mat2cell(y_y_values(1:end)',1, ones(length(y_y_values),1));
...
for i = 1:2%length(referenca_x)
    for j = 1:length(referenca_y)
    x_0 = 0.03 + (0.36-0.03)*rand();
    y_0 = 0.03 + (0.46-0.03)*rand();
    disp('referenca')
    i
    [u_x_values, u_y_values, y_x_values, y_y_values] = simuliraj_referenca(referenca_x(i), referenca_y(j),x_0, y_0);
    C = mat2cell(y_x_values(1:end)',1, ones(length(y_x_values),1));
    C1 = mat2cell(y_y_values(1:end)',1, ones(length(y_y_values),1));
    y_mul = catsamples(y_mul, C,C1,'pad');
